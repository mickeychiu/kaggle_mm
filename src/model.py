...
X_test = pd.read_csv(data_path + "X-test.csv").drop(["Unnamed: 0"], axis=1)
...
def predict(X_test, model, title):
    ...
    preds_frame.to_csv(data_path + title + ".csv", index=False)

    return preds_frame

def bracket(preds, title):
    names = pd.read_csv(data_path + "MTeams.csv")
    ...
    mapped.to_csv(data_path + title + ".csv", index=False)
